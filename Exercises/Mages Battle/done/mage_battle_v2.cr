class Mage
  ...
  property mp : Int32
  property spellbook
  @evolution_counter : Hash(Spell, Int32)
  ...
  def initialize(name : String)
    ...
    @evolution_counter = Hash(Spell, Int32).new
  end
  ...
  def cast_spell(spell : Spell, target : Mage)
    @evolution_counter[spell] ||= 0
    @evolution_counter[spell] += 1

    if @evolution_counter[spell] == 3
      @spellbook.evolve_spell(spell, (spell.mp_cost / 1.5).to_i32, (spell.damage * 1.5).to_i32, spell.description + " (evolved)", spell.name + " (evolved)")
      puts "#{spell.name} has evolved!"
      @evolution_counter[spell] = 0
    end
...
class Spellbook
  ...
  def evolve_spell(spell : Spell, new_mp_cost : Int32 = spell.mp_cost, new_damage : Int32 = spell.damage, new_description : String = spell.description, new_name : String = spell.name)
    spell.evolve(new_name, new_mp_cost, new_damage, new_description)
  end
...
class Spell
  ...
  def evolve(new_name : String = @name, new_mp_cost : Int32 = @mp_cost, new_damage : Int32 = @damage, new_description : String = @description)
    @name = new_name
    @mp_cost = new_mp_cost
    @damage = new_damage
